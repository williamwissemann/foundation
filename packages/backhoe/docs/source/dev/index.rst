...
The backhoe project implements a regression
...
not broken existing functionality. The backhoe
...
``backhoe`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``backhoe`` package can be found.

.. code-block:: console

    (backhoe) $ cd tests
    (backhoe) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (backhoe) $ python -m test_version
...
.. code-block:: console
    ...
    (backhoe) $ make check-static-analysis
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/backhoe/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (backhoe) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (backhoe) $ make dist-upload

  The package should now be available at https://pypi.org/project/backhoe/
